...
push — a release happens only when an operator pushes a SemVer tag (or runs a
...
1. Confirm `main` is at the exact reviewed commit and green in CI (the
   `host-checks` job plus the `verify` job — the same clean build, static/unit,
   and offscreen golden-verifier gates the release re-runs).
...
> release inputs. The two independent clean builds above are the reproducibility
> check that confirms the pinned toolchain builds the reviewed source before any
